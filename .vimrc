...
highlight StatusLineNC guifg=Gray guibg=darkred
...
"""""""""""""""""""""""""'"""""""""""""""""""""""""""""""""""""""
"新建.c,.cpp,.h,.sh,.qml
au BufRead,BufNewFile *.qml setfiletype qml "新建vim默认不知道的文件类型以做判断
autocmd BufNewFile *.cpp,*.[ch],*.sh,*.qml exec ":call SetTitle()"
"定义SetTitle函数，自动根据文件后缀名插入文件头
func SetTitle()
 "如果文件后缀名为'sh'
 if &filetype=='sh'
    call setline(1,"\#######################################################")
    call append(line("."),"\#File Name: " .expand("%"))
    call append(line(".")+1,"\#Author: Match")
    call append(line(".")+2,"\#Email: Match.YangWanQing@gmail.com")
 ...
 if &filetype=='qml'
    ...
    call append(line(".")+5,"\*")
    call append(line(".")+6,"\*************************************************************/")
    call append(line(".")+7,"\import QtQuick 2.1")
    call append(line(".")+8,"")
 endif
endfunc
